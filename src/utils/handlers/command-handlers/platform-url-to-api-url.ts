...
const regExpSevenTVEmoteNotInSet: Readonly<RegExp> = new RegExp(
  /^https:\/\/7tv\.app\/emotes\/[A-Za-z0-9]+$/
);
const regExpOldSevenTVEmoteNotInSet: Readonly<RegExp> = new RegExp(
  /^https:\/\/old\.7tv\.app\/emotes\/[A-Za-z0-9]+$/
);

export async function sevenTVUrlToSevenTVNotInSet(
  url: string
): Promise<SevenTVEmoteNotInSet | undefined> {
  const urlSplit: readonly string[] = url.split(SPLITTER);

  if (!regExpSevenTVEmoteNotInSet.test(url))
    if (!regExpOldSevenTVEmoteNotInSet.test(url)) return undefined;
